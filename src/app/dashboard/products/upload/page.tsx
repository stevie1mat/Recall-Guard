...
import { ArrowLeft, Upload, FileType, CheckCircle } from 'lucide-react'
  ...
  async function handleSubmit(formData: FormData) {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // After 2 seconds, router pushes automatically if handled by action, but our action redirects.
        // The redirect from server action throws an error that Next.js catches, so the lines below might not run if redirect succeeds.
        // Wait, if it redirects, it's fine.
      }
    } catch (e: any) {
